...
public class Keyboard
{
    ...
    public static final Keyboard KEYPAD
                                 = new Keyboard("keypad", new AlignedAdjacentAdjacentGraphBuilder(loadAsString(
                                                "keypad.txt")));

    public static final Keyboard MAC_KEYPAD
                                 = new Keyboard("mac_keypad", new AlignedAdjacentAdjacentGraphBuilder(loadAsString(
                                                "mac_keypad.txt")));
    ...
    private static String loadAsString(final String name)
    {
        try
        {
            final InputStream input = RESOURCE_LOADER.getInputStream(RESOURCES_PACKAGE_PATH + name);
            try
            {
    ...
    static abstract class AdjacentGraphBuilder
    {
        ...
        /**
         * builds an adjacency graph as a dictionary: {character: [adjacent_characters]}. adjacent characters occur in a
         * clockwise order. for example: on qwerty layout, 'g' maps to ['fF', 'tT', 'yY', 'hH', 'bB', 'vV'] on keypad
         * layout, '7' maps to [None, None, None, '=', '8', '5', '4', None] *
         */
        public Map<Character, List<String>> build()
        {
            final Map<Position, String> positionTable = buildPositionTable(layout);

            final Map<Character, List<String>> adjacencyGraph = new HashMap<Character, List<String>>();
            for (Map.Entry<Position, String> entry : positionTable.entrySet())
            {
                for (final char key : entry.getValue().toCharArray())
                {
                    final List<String> adjacencies = new ArrayList<String>();
        ...
        {
            final Map<Position, String> positionTable = new HashMap<Position, String>();
    ...
    static class SlantedAdjacentGraphBuilder extends AdjacentGraphBuilder
    {
        ...
        /**
         * returns the six adjacent coordinates on a standard keyboard, where each row is slanted to the right from the
         * last. adjacencies are clockwise, starting with key to the left, then two keys above, then right key, then two
         * keys below. (that is, only near-diagonal keys are adjacent, so g's coordinate is adjacent to those of
         * t,y,b,v, but not those of r,u,n,c.)
         */
        @Override
